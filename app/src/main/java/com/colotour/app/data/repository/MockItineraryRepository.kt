...
import com.colotour.app.domain.engine.ItineraryImageEnricher
...
class MockItineraryRepository : ItineraryRepository {
    ...
    private val imageRepository = WikimediaPlaceImageRepository()
    private val imageEnricher = ItineraryImageEnricher(imageRepository)
    ...
    override suspend fun generarItinerario(preferences: TravelPreferences): Result<Itinerary> {
        ...
        return try {
            if (preferences.destino.isBlank()) {
                ...
                val enriched = imageEnricher.enrich(itinerary)
